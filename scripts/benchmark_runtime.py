...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="flint-bench-") as temp_dir_raw:
...
def _measure_incremental_feedback_cycle(*, project_dir: Path, repo_root: Path) -> float:
    ...
    started = time.perf_counter()
    _run(
        [sys.executable, "-m", "flint", "lint", "check", "tests/test_health.py"],
        cwd=project_dir,
        env=env,
        timeout=120,
    )
    _run(
        [sys.executable, "-m", "flint", "test", "tests/test_health.py"],
        cwd=project_dir,
        env=env,
        timeout=120,
    )
...
def _set_run_port(config_path: Path, port: int) -> None:
    ...
    if not replaced:
        raise RuntimeError("Could not locate `[run].port` in flint.toml")
